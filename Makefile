...
tests:
	mkdir -p storage/framework/sessions && chmod -R 777 storage/framework/sessions && docker-compose run nodejs sh -c "cd /usr/src/app && npm install -g jasmine-node && jasmine-node spec/oauth ; (exit $?)"
